...
namespace EcomMicroservice2.Models
{
    public static class QueryStringClass
    {
        ...
        public static string getAllProductWithDetailsNoFilter = " Select FAMILY_NAME, CLASS_NAME, COMMODITY, COMMODITY_NAME,SKU.ITEM_NUMBER, SKU.DESCRIPTION,SKU.LONG_DESCRIPTION,BRAND,SKU_ATTRIBUTE_VALUE1  AS SIZE, SKU_ATTRIBUTE_VALUE2  AS COLOUR, 0.0 AS LIST_PRICE, 5 AS DISCOUNT,'Yes' AS IN_STOCK, '09-09-2019' AS PRICE_EFFECTIVE_DATE " +
                                                        " FROM XXIBM_PRODUCT_CATALOGUE AS CATALOGUE " +
                                                        " LEFT JOIN XXIBM_PRODUCT_SKU AS SKU ON CATALOGUE.COMMODITY = SKU.CATALOGUE_CATEGORY " +
                                                        " LEFT JOIN XXIBM_PRODUCT_STYLE AS STYLE ON CATALOGUE.COMMODITY = STYLE.CATALOGUE_CATEGORY "+
